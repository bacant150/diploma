...
class UserProfilesRepository:
    ...
    def load_all(self) -> list[dict[str, Any]]:
        raw_profiles = read_json_list(
            self.file_path,
            logger=logger,
            label="user_profiles.json",
        )
    ...
    def write_all(self, profiles: list[dict[str, Any]]) -> None:
        ...
        logger.info(
            "Записано профілі користувачів: path=%s count=%s",
            self.file_path,
    ...
    def prepare_for_dashboard(
        ...
        *,
        saved_builds_by_id: dict[str, dict[str, Any]] | None = None,
    ) -> dict[str, Any]:
    ...
    def prepare_history_entry(
        self,
        entry: dict[str, Any],
        builds_by_id: dict[str, dict[str, Any]],
    ) -> dict[str, Any]:
        prepared = dict(entry)
        ...
        prepared["has_snapshot"] = isinstance(prepared.get("result_snapshot"), dict) and bool(
            prepared.get("result_snapshot")
        )
        prepared["open_url"] = (
            f"/profile/history/{prepared.get('id')}" if prepared.get("id") else None
        )
        prepared["delete_url"] = (
            f"/profile/history/{prepared.get('id')}/delete"
            if prepared.get("id")
            else None
        )
        return prepared
