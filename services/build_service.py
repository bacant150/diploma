...
def _prepare_public_alternatives(raw_alternatives: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any] | None]:
...
def build_configuration_from_form(form: Any) -> tuple[dict[str, Any], dict[str, Any]]:
    try:
        ...
        raise HTTPException(status_code=400, detail='; '.join(validation_error_messages(exc))) from exc
    ...
    raw_alternatives = _run_alternative_builds(validated_result, payload, budget_mode=budget_mode)
    public_alternatives, alternative_primary_result = _prepare_public_alternatives(raw_alternatives)
