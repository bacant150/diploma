...
def _describe_cpu(part: Part, purpose: str, context: Dict[str, Any], parts: Dict[str, Part]) -> str:
    ...
    if purpose == "gaming":
        ...
        if gpu:
            ...
            base += (
                " Дискретна відеокарта в цій збірці не використовується: графічне навантаження "
                "покриває інтегрована графіка процесора. Для обраного стартового або невибагливого "
                "ігрового сценарію цього достатньо; для важких AAA-ігор на високих налаштуваннях "
                "доцільно обрати режим із дискретною відеокартою."
            )
